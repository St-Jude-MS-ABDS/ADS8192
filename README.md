...
Scientific Applications**. This R package demonstrates the "three interfaces,
one core" architecture for scientific software.
...
All three interfaces call the **same core functions** — no duplicated logic.
...
| Function | Purpose |
|----------|---------|
| `make_se()` | Create SummarizedExperiment from counts + metadata |
| `top_variable_features()` | Select N most variable genes |
| `run_pca()` | Run PCA, return scores + metadata |
| `pca_variance_explained()` | Variance % per PC |
| `plot_pca()` | PCA scatter plot (ggplot2) |
| `save_pca_results()` | Export results to TSV files |
| `run_app()` | Launch Shiny PCA Explorer |
...
- **[HW1 Grading Rubric](Unit1/Assessments/HW1_Rubric.md)** (25 points)
