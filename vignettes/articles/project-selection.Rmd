...
[UMAP](https://alleninstitute.org/resource/what-is-a-umap/) is a nonlinear dimensionality reduction method that reveals
...
sample groupings with additional metadata annotations.
...
One of the most bog standard 'omics analyses is differential expression analysis.
...
results are tried and true visualizations for count-based data.
...
Quality control is the essential first step in scRNA-seq
...
Rather than analyzing genes individually, biologists often ask
...
Raw counts must be normalized for library size (and composition bias, though we skip that here) before cross-sample comparison.
...
The genes-by-samples heatmap is one of the most iconic figures
...
Determining the meaningful number of principal components after PCA is a common task.
...
Batch effects (systematic technical variation from processing
