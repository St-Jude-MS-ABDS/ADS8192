...
Students will offer three interfaces to the same core functionality from the package:

1. **R API** — exported functions with roxygen2 documentation
2. **Shiny app** — interactive analysis/visualization
3. **CLI** — command-line interface via [Rapp](https://github.com/r-lib/Rapp)
...
- A Shiny app that allows users to interactively explore/analyze/plot the data and results
...
If you see function calls you aren't familiar with in the raw code blocks, looking them on in R is your friend, e.g. `?prcomcp`.
...
**Shiny inputs:** Number of top genes, log-transform toggle, scale toggle, PC
...
**Shiny inputs:** Correlation method, linkage method, k slider, n_top slider,
...
**CLI outputs:** `de_results.tsv`, `de_summary.tsv`, `filtering_diagnostics.tsv`
...
**Shiny inputs:** n_top slider, max_k slider, selected k slider, toggle between
...
**Shiny inputs:** MAD threshold slider, minimum genes input, max spike-in %
...
**Shiny inputs:** Select built-in sets or upload GMT, score method dropdown,
...
- Normalize raw counts using `scuttle::logNormCounts()` — the standard
  Bioconductor approach (library-size-based size factors + log2 transform)
...
**Shiny inputs:** Gene name text input (comma-separated), grouping variable
...
**Shiny inputs:** Correlation threshold slider, method dropdown, n_top slider,
...
**Shiny inputs:** n_top slider, scale method dropdown, gene_k slider, metadata
...
**Shiny inputs:** n_top slider, checkboxes to toggle overlay of each method,
...
**Shiny inputs:** Batch column dropdown, biological column dropdown, n_top
...
**Shiny inputs:** Grouping variable dropdown, n_markers slider, FC threshold
