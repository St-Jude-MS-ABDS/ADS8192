...
**Dependencies:** `SingleCellExperiment`, `cluster`, `scRNAseq` (data only)
...
**Rationale:** Raw counts must be normalized for library size (and composition bias, though we skip that here) before cross-sample comparison.
Different methods (CPM, log2-CPM, quantile, upper-quartile) make different
assumptions. Visualizing how each method reshapes per-sample distributions
can be informative.
...
delta, acinar, ductal, and more. The dramatic library size variation across
cells (typical of scRNA-seq) makes normalization effects visually striking —
CPM, log2-CPM, and quantile normalization reshape per-cell distributions in
very different ways.
...
- Apply multiple normalization methods (CPM, log2-CPM, quantile,
  ...
  assays in the SCE
- Compute per-cell distributional statistics (median, IQR, CV) before and
  after each normalization method
...
- Visualize side-by-side distributions across methods as boxplots or density
  curves
- Export normalized counts, normalization statistics, and method agreement
  matrix to TSV files

**Key parameters:** `methods` (character vector: "cpm", "log2", "quantile",
"size_factor"), `min_count`

**CLI outputs:** `normalized_counts.tsv`, `normalization_stats.tsv`,
`method_agreement.tsv`

**Shiny inputs:** Checkbox group to select methods, boxplot ↔ density toggle,
per-cell or per-method view, agreement heatmap
