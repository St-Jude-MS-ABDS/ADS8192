...
3. Explain why packaging, optional dependencies, and deployment are reproducibility decisions as much as UI decisions
...
This file exists for deployment platforms like Posit Cloud that expect a standalone `app.R` entry point. Rather than calling internal package functions directly (which requires `:::` and is bad practice), it uses `return_as_list = TRUE` to get the UI and server back as a plain list, then passes them to `shinyApp()`.
...
- The app works on any `SummarizedExperiment` — no extra data packages required
...
3. Use `inst/app/app.R` (already in your repo) which calls `run_app(se = example_se, return_as_list = TRUE)`
