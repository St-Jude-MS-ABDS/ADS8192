...
`setValidity()` lets you define invariants that are checked every time
`validObject()` is called (and optionally on every `new()`):
...
For this course we rebuild a tiny example to make the design visible. In professional work, the default should be to reuse and extend an existing ecosystem type unless you have a concrete reason not to.
