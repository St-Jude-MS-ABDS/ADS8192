...
def quiz05():
    questions = [
        ...
        },
        {
            "num": 4,
            "type": "Short Answer",
            "text": "A colleague puts their Shiny dependency in Imports. You suggest moving it to Suggests. In 1–2 sentences, explain why Suggests is more appropriate when the Shiny app is an optional feature of the package.",
        },
        {
            "num": 5,
            "type": "Multiple Choice",
...
def quiz07():
    questions = [
        ...
        },
        {
            "num": 4,
            "type": "Short Answer",
            "text": "Explain in 1–2 sentences why Shiny's reactive programming model is described as 'declarative' rather than 'imperative.' What does this mean for how you structure app logic?",
        },
    ]
    ...
    answers = [
        ...
        {"num": 4, "text": "In declarative programming, you describe relationships between values (e.g., 'this plot depends on these inputs') rather than writing step-by-step instructions for when to update. Shiny's reactive graph automatically determines what to recompute and when, so you focus on what depends on what—not the order of execution."},
    ]
...
def quiz08():
    questions = [
        ...
        },
        {
            "num": 4,
            "type": "Short Answer",
            "text": "Explain in 1–2 sentences why system.file() should be used to locate package files at runtime instead of constructing paths manually.",
        },
        {
            "num": 5,
            "type": "Multiple Choice",
            "text": "When deploying a packaged Shiny app to shinyapps.io, what must be true about the deployment compared to local development?",
            "choices": [
                ("A", "The deployed app will automatically have access to all packages on your local machine"),
                ("B", "All dependencies must be explicitly declared, since the server installs them fresh from DESCRIPTION"),
                ("C", "The server uses Docker, so no R packages are needed"),
                ("D", "Shiny apps cannot be deployed from within R packages"),
            ],
        },
    ]
    ...
    answers = [
        ...
        {"num": 4, "text": "system.file() resolves the installed location of package files at runtime, which varies by platform, R version, and library path. Hardcoded paths break as soon as the package is installed somewhere other than where you developed it."},
        {"num": 5, "text": "B. Deployment servers start with a clean environment and install only what's declared in your package's dependencies. If a package isn't in Imports or Suggests (and properly checked for), the deployed app will fail even though it worked locally."},
    ]
...
def quiz10():
    questions = [
        ...
        },
        {
            "num": 4,
            "type": "Short Answer",
            "text": "Your CLI works during development but fails after the package is installed. In 1–2 sentences, explain the most common reason and the design principle that prevents it.",
        },
    ]
    ...
    answers = [
        ...
        {"num": 4, "text": "The most common cause is using relative or development paths to locate package files (e.g., inst/exec/script.R) instead of system.file(), which resolves installed paths at runtime. The design principle: always use system.file() to locate package resources and never assume a particular installation path."},
    ]
...
def quiz11():
    questions = [
        ...
        },
        {
            "num": 4,
            "type": "Short Answer",
            "text": "For the Homework 1 package, all three interfaces (R functions, Shiny app, CLI) must work. In 1–2 sentences, explain why they should share the same analysis core rather than each implementing the analysis independently.",
        },
        {
            "num": 5,
            "type": "Multiple Choice",
    ...
    answers = [
        ...
        {"num": 4, "text": "A shared analysis core means there is exactly one implementation of each computation. If a bug is found, fixing it in the core function fixes it everywhere; if each interface implements the analysis independently, bugs can exist in one interface but not others, and behavior can diverge silently over time."},
        {"num": 5, "text": "B. Actionable, specific feedback about incorrect behavior is the most valuable kind of review. It identifies a concrete problem the author can fix and test. Stylistic preferences and language suggestions are subjective and less impactful."},
    ]
